...
then
#echo 1 #`/sbin/runuser -l mano -c  'docker cp /opt/PLAO/vnf_price_list.yaml $(docker ps -qf name=osm_pla):/placement/'`
...
then
#echo 2 # `/sbin/runuser -l mano -c  'docker cp /opt/PLAO/pill_price_list.yaml $(docker ps -qf name=osm_pla):/placement/'`
